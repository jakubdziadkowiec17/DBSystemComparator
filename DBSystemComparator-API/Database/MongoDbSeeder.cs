...
namespace DBSystemComparator_API.Database
{
    public static class MongoDbSeeder
    {
        ...
        {
            await context.Clients.Indexes.CreateManyAsync(new[]
            {
                ...
                new CreateIndexModel<ClientCollection>(
                    Builders<ClientCollection>.IndexKeys.Ascending(c => c.DateOfBirth)
                ),
                new CreateIndexModel<ClientCollection>(
                    Builders<ClientCollection>.IndexKeys
                        .Ascending(c => c.IsActive)
                        .Ascending(c => c.DateOfBirth)
                )
            });

            await context.Rooms.Indexes.CreateManyAsync(new[]
            {
                ...
                new CreateIndexModel<RoomCollection>(
                    Builders<RoomCollection>.IndexKeys.Ascending(r => r.Capacity)
                        .Ascending(r => r.PricePerNight)
                ),
                new CreateIndexModel<RoomCollection>(
                    Builders<RoomCollection>.IndexKeys.Ascending(r => r.PricePerNight)
                )
            });

            await context.Services.Indexes.CreateManyAsync(new[]
            {
            ...
            await context.Reservations.Indexes.CreateManyAsync(new[]
            {
                ...
                new CreateIndexModel<ReservationCollection>(
                    Builders<ReservationCollection>.IndexKeys.Ascending("client.isActive")
                        .Ascending("client.dateOfBirth")
                ),
                new CreateIndexModel<ReservationCollection>(
                ...
                new CreateIndexModel<ReservationCollection>(
                    Builders<ReservationCollection>.IndexKeys.Ascending("room.pricePerNight")
                ),
                new CreateIndexModel<ReservationCollection>(
