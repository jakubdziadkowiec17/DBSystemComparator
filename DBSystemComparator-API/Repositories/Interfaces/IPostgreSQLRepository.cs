...
namespace DBSystemComparator_API.Repositories.Interfaces
{
    public interface IPostgreSQLRepository
    {
        ...
        Task<int> CreateRoomAsync(int number, int capacity, double pricePerNight, bool isActive);
        ...
        Task<List<int>> CreateRoomsAsync(int number, int capacity, double pricePerNight, bool isActive, int count);
        // READ
        ...
        Task CreateRoomsBatchAsync(IEnumerable<(int number, int capacity, double pricePerNight, bool isActive)> rooms);
